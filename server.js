...
app.get('/player_info/post', function(req, res) {
	var num = req.query.player_choice //.toString();
	var query1 = "SELECT year, major, passing_yards, rushing_yards, receiving_yards FROM football_players WHERE id=" + num + ";";
	var query2 = "with elements (element) as (select unnest(ARRAY(select players from football_games))) select count(*) from elements where element=" + num + ";";
	var query3 = 'SELECT name, id FROM football_players;';
	db.task('get-everything', task => {
	  return task.batch([
		  task.any(query1),
		  task.any(query2),
